...
export default function Tooltip({ content, children, side = 'top', wrapperClassName }: TooltipProps) {
  ...
  useEffect(() => {
    ...
    const position = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const placeTop = (side === 'top' && tip.height + padding <= spaceAbove) || tip.height + padding > spaceBelow;
        let top = placeTop ? tri.top - tip.height - 8 : tri.bottom + 8;
        let left = centerX - tip.width / 2;
        if (left < padding) left = padding;
        if (left + tip.width > window.innerWidth - padding) left = window.innerWidth - padding - tip.width;
      ...
      {open &&
        createPortal(
          <div
            ...
            id={id}
            ref={tipRef}
            className="p-fixed p-z-[10002] p-w-auto p-rounded-md p-bg-gray-900 p-px-3 p-py-2 p-text-xs p-font-medium p-text-white p-shadow-xl p-break-words"
            style={{
